  ...
  for warmup_ratio in 0.04; do
    ...
    for lr in 5e-5; do
      ...
      for patch_image_size in 480; do
        ...
        CUDA_VISIBLE_DEVICES=4  python3 -m torch.distributed.launch --nproc_per_node=${GPUS_PER_NODE} --nnodes=${WORKER_CNT} --node_rank=${RANK} --master_port=${MASTER_PORT} train.py \
            ${data} \
